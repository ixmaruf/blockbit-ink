...
/**
 * Auto-Append New Batch of 19 GDT Wallets and sync Dual Status
 */
function addBatch19() {
  const wallets = ["0xa5662263f0af0da991fae81ded3ad570a63c7f12", "0xD28bb42397e3DEc97bD6E2d52f6728A6729Be500", "0xdb44ecda39D75C1854d97951902a675990d7675F", "0xe3588FE1e0025853c5735082DA2ff3e0b1aA1348", "0x547651bb98b35392ccf00fe502b5e11bffe7521a", "0x9eff056964bdff91dc8ef16afde18bfab5ccb616", "0x4ec26258c9ac012d3f2becee83e0ebd81d799d79", "0x4F91E3feC5c849ccb99351c97329BeADB4E3C004", "0xE3F0E67441C43FCd713f15fdCc5e2d9153C0b2FA", "0xe0efbD5B6Fa5006014C24d61c42dEAAbFD5A71D2", "0xb60060d07143748b19e6f4afbf97faa1bb7fe1ea", "0x5c1a266bc2e61ab2ed1242d90b4b8bb477cc83ea", "0x49762F06057fb04965aE3aBbc1D9EfDA6C903326", "0x781663604ede0b172c69ef89641fefa50c466836", "0x7e17d0493CD8131A7dC3712F16dbBe4Db3cFF9d7", "0xeC21f5D5B41A745a421A2879CC70d27e3187A248", "0xB460f9298D36A600e7E1F4Fa6DF34DF8cbCCe12f", "0x0f5b0Ff9fb4Ee1E0A518806e25B1FE70170D6C34", "0xc0aab4bcdfa1fc2f2b10daf27d76bde8a825d162", "0x772BABbbB69Dd876337938732E8deaa39ACbCdCB", "0x92eb5788de4449990927cd05bec73740a10b6bee"];
  const ss = getSpreadsheet_();
  const gdtSheet = ss.getSheetByName(GDT_SHEET);
  const fcfsSheet = ss.getSheetByName(FCFS_SHEET);
  
  const existingGdt = new Set();
  if (gdtSheet.getLastRow() > 1) {
    const data = gdtSheet.getRange(2, 3, gdtSheet.getLastRow() - 1, 1).getValues();
    for (let i = 0; i < data.length; i++) {
      const w = String(data[i][0] || '').toLowerCase().trim();
      if (w) existingGdt.add(w);
    }
  }
  
  if (fcfsSheet && fcfsSheet.getLastRow() > 1) {
    const fcfsRange = fcfsSheet.getRange(2, 3, fcfsSheet.getLastRow() - 1, 4);
    const fcfsValues = fcfsRange.getValues();
    let updatedFcfs = 0;
    for (let i = 0; i < fcfsValues.length; i++) {
      const w = String(fcfsValues[i][0] || '').toLowerCase().trim();
      if (wallets.map(x => x.toLowerCase()).includes(w)) {
        fcfsValues[i][3] = 'Dual Allocation: GDT + FCFS';
        updatedFcfs++;
      }
    }
    if (updatedFcfs > 0) {
      fcfsRange.setValues(fcfsValues);
    }
  }
  ...
  for (let i = 0; i < wallets.length; i++) {
    const addr = wallets[i].trim().toLowerCase();
    if (!addr || existingGdt.has(addr)) continue;
    existingGdt.add(addr);
    added++;
    rows.push([nowStr, '', addr, 'DC-' + hex4_() + '-' + hex4_(), 'GDT Guaranteed Allocation', 'Dual Allocation: GDT + FCFS']);
  }
  if (rows.length > 0) {
    gdtSheet.getRange(gdtSheet.getLastRow() + 1, 1, rows.length, 6).setValues(rows);
  }
  
  const cache = CacheService.getScriptCache();
  for (let i = 0; i < wallets.length; i++) {
    cache.remove('alloc_' + wallets[i].toLowerCase());
  }
  
  Logger.log('Added to GDT: ' + added + ', Total GDT rows: ' + (gdtSheet.getLastRow() - 1));
  return { added: added, total: gdtSheet.getLastRow() - 1 };
}
