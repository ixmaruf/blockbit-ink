...
class ThreeVillageWorld {
  ...
  // ==========================================
  // SCULPTURAL ANIME FOLIAGE (PERIMETER HILLS & LATE-SEASON BRANCHES)
  // ==========================================
  buildFoliage() {
    // 1. Tall Majestic Branching Trees along Outer Hills and Perimeter (Village Center Unobstructed)
    const perimeterTreePositions = [
      // West Ridge & Hills
      { x: -24, z: 12, type: 'cherry', scale: 1.45 },
      { x: -22, z: -4, type: 'autumn', scale: 1.4 },
      { x: -25, z: -18, type: 'green', scale: 1.5 },
      { x: -20, z: 24, type: 'cherry', scale: 1.35 },
      // East Ridge & Hills
      { x: 24, z: 14, type: 'green', scale: 1.45 },
      { x: 22, z: -10, type: 'cherry', scale: 1.4 },
      { x: 25, z: -20, type: 'autumn', scale: 1.5 },
      { x: 21, z: 22, type: 'autumn', scale: 1.35 },
      // North & Distant Background Hills
      { x: -14, z: -25, type: 'cherry', scale: 1.4 },
      { x: 0, z: -28, type: 'autumn', scale: 1.5 },
      { x: 14, z: -26, type: 'green', scale: 1.45 },
      // Far River Edges (Distant South & North)
      { x: -8, z: 28, type: 'green', scale: 1.3 },
      { x: 8, z: 28, type: 'cherry', scale: 1.3 },
      { x: -9, z: -26, type: 'autumn', scale: 1.3 },
      { x: 9, z: -26, type: 'green', scale: 1.3 }
    ];

    perimeterTreePositions.forEach(cfg => {
    ...
    // 2. Weeping Willow Spirit Trees along Outer Riverbanks
    const willowPositions = [
      { x: -8.2, z: 16, scale: 1.35 },
      { x: 8.2, z: 16, scale: 1.35 },
      { x: -8.2, z: -20, scale: 1.3 },
      { x: 8.2, z: -20, scale: 1.3 }
    ];
    ...
    // 3. Mountain Evergreen Pines along High Ridges
    const pinePositions = [
      [-26, 5.5, 6], [-28, 6.5, -8], [-24, 5.0, 22], [26, 5.5, 4],
      [28, 6.5, -14], [-27, 7.0, -18], [27, 7.0, -16], [-12, 4.5, -28],
      [12, 4.5, -28], [-25, 6.0, 28], [25, 6.0, 28]
    ];

    pinePositions.forEach((pos, idx) => {
      const pine = this.createDetailedPineTree();
      pine.position.set(pos[0], pos[1], pos[2]);
      const s = 0.95 + (idx % 3) * 0.2;
      pine.scale.setScalar(s);
      this.foliageGroup.add(pine);
    });

    // 4. Low Garden Bushes & Wildflower Clumps (Keeps village open & beautiful)
    ...
    for (let b = 0; b < 22; b++) {
      ...
      for (let bc = 0; bc < 3; bc++) {
        const clump = new THREE.Mesh(
          new THREE.DodecahedronGeometry(0.45 + Math.random() * 0.3, 1),
          bushGreenMat
        );
        clump.position.set((Math.random() - 0.5) * 0.6, 0.3 + Math.random() * 0.25, (Math.random() - 0.5) * 0.6);
      ...
      for (let fl = 0; fl < 5; fl++) {
        ...
        blossom.position.set((Math.random() - 0.5) * 0.9, 0.45 + Math.random() * 0.35, (Math.random() - 0.5) * 0.9);
        bush.add(blossom);
      }
  ...
  // ==========================================
  // SCULPTURAL TALL BRANCHING ANIME TREE (SEAMLESS SOLID JOINTS & BLOSSOM BUDS)
  // ==========================================
  createDetailedBranchingTree(type = 'cherry') {
    const tree = new THREE.Group();

    let barkTexture = this.textures.barkSakura;
    let budColor = 0xf472b6;
    ...
    if (type === 'autumn') {
      ...
      barkTexture = this.textures.barkBirch;
      budColor = 0xfde047;
      leafColor = 0x4ade80;
    }
    ...
    const leafSprigMat = new THREE.MeshStandardMaterial({
      color: leafColor,
      roughness: 0.6
    });

    // 1. Organic Root Spurs Arching into Soil
    for (let r = 0; r < 4; r++) {
      const rootAng = (r / 4) * Math.PI * 2 + (Math.random() - 0.5) * 0.3;
      const rootGeo = new THREE.CylinderGeometry(0.12, 0.32, 1.4, 6);
      rootGeo.translate(0, 0.7, 0);
      const root = new THREE.Mesh(rootGeo, barkMat);
      root.position.set(Math.cos(rootAng) * 0.5, 0.05, Math.sin(rootAng) * 0.5);
      root.rotation.x = Math.sin(rootAng) * 0.55;
      root.rotation.z = -Math.cos(rootAng) * 0.55;
      root.castShadow = true;
      tree.add(root);
    }

    // 2. Tall Gnarled Sculptural Trunk with Solid Base
    const trunkGeo = new THREE.CylinderGeometry(0.32, 0.62, 4.8, 8);
    trunkGeo.translate(0, 2.4, 0); // Origin at ground (y = 0)
    const trunk = new THREE.Mesh(trunkGeo, barkMat);
    trunk.castShadow = true;
    trunk.receiveShadow = true;
    tree.add(trunk);

    // Helper: Create Seamless Connected Branch Limb
    const createSeamlessBranch = (len, rBot, rTop) => {
      const group = new THREE.Group();
      // Socket Knuckle Sphere at joint so no gap exists between parent and child
      const knuckle = new THREE.Mesh(new THREE.SphereGeometry(rBot * 1.12, 7, 7), barkMat);
      knuckle.castShadow = true;
      group.add(knuckle);

      // Tapered branch cylinder with origin (0, 0, 0) at the bottom base
      const cylGeo = new THREE.CylinderGeometry(rTop, rBot, len, 6);
      cylGeo.translate(0, len / 2, 0);
      const cyl = new THREE.Mesh(cylGeo, barkMat);
      cyl.castShadow = true;
      group.add(cyl);

      return { group, len, rTop };
    };

    // 3. 6 Primary Spreading Boughs Anchored Directly into Trunk
    const boughDefs = [
      { y: 2.7, rotZ: 0.62, rotY: 0.2, len: 3.2, rBot: 0.24, rTop: 0.16 },
      { y: 3.3, rotZ: -0.68, rotY: 1.8, len: 3.0, rBot: 0.22, rTop: 0.15 },
      { y: 3.8, rotX: 0.65, rotY: 3.3, len: 2.8, rBot: 0.20, rTop: 0.14 },
      { y: 4.2, rotX: -0.60, rotY: 4.8, len: 2.6, rBot: 0.18, rTop: 0.13 },
      { y: 4.5, rotZ: 0.45, rotY: 0.9, len: 2.4, rBot: 0.16, rTop: 0.12 },
      { y: 4.8, rotZ: -0.42, rotY: 2.5, len: 2.2, rBot: 0.15, rTop: 0.11 }
    ];

    boughDefs.forEach(b => {
      const bough = createSeamlessBranch(b.len, b.rBot, b.rTop);
      bough.group.position.set(0, b.y, 0);
      if (b.rotX) bough.group.rotation.x = b.rotX;
      if (b.rotZ) bough.group.rotation.z = b.rotZ;
      bough.group.rotation.y = b.rotY;

      // 4. Secondary Sub-Twig Fork connected solidly along parent bough
      const subTwig1 = createSeamlessBranch(b.len * 0.55, b.rTop * 0.95, 0.06);
      subTwig1.group.position.set(0, b.len * 0.65, 0);
      subTwig1.group.rotation.z = 0.55;
      subTwig1.group.rotation.y = 0.4;
      bough.group.add(subTwig1.group);

      // Tertiary Sub-Twig Fork
      const subTwig2 = createSeamlessBranch(b.len * 0.42, b.rTop * 0.85, 0.05);
      subTwig2.group.position.set(0, b.len * 0.45, 0);
      subTwig2.group.rotation.z = -0.50;
      subTwig2.group.rotation.x = 0.35;
      bough.group.add(subTwig2.group);

      // 5. Blossom Buds & Fresh Leaf Sprigs attached directly to nodes & tips
      const tipPoints = [
        new THREE.Vector3(0, b.len, 0),
        new THREE.Vector3(0, b.len * 0.55, 0).applyEuler(subTwig1.group.rotation).add(subTwig1.group.position),
        new THREE.Vector3(0, b.len * 0.42, 0).applyEuler(subTwig2.group.rotation).add(subTwig2.group.position)
      ];

      tipPoints.forEach(tip => {
        // Glowing blossom bud
        for (let bd = 0; bd < 3; bd++) {
          const bud = new THREE.Mesh(new THREE.SphereGeometry(0.16 + Math.random() * 0.1, 6, 6), budMat);
          bud.position.set(
            tip.x + (Math.random() - 0.5) * 0.45,
            tip.y + (Math.random() - 0.5) * 0.35,
            tip.z + (Math.random() - 0.5) * 0.45
          );
          bough.group.add(bud);
        }

        // Small fresh leaf sprig
        const sprig = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.04, 0.16), leafSprigMat);
        sprig.position.set(
        ...
        bough.group.add(sprig);
      });

      tree.add(bough.group);
    });

    // 6. Crown Branch Crest at Top of Trunk
    for (let cr = 0; cr < 6; cr++) {
      const crownBud = new THREE.Mesh(new THREE.SphereGeometry(0.22 + Math.random() * 0.12, 6, 6), budMat);
      const ang = (cr / 6) * Math.PI * 2;
      crownBud.position.set(
        Math.cos(ang) * 0.65,
        5.1 + Math.sin(cr * 1.5) * 0.4,
        Math.sin(ang) * 0.65
      );
      tree.add(crownBud);
    }
  ...
  // ==========================================
  // ANCIENT SPIRIT TREE (TIGHTLY WRAPPED SPIRAL VINES & ANCHORED TENDRILS)
  // ==========================================
  createWeepingWillowTree() {
    const willow = new THREE.Group();
    const barkMat = new THREE.MeshStandardMaterial({ map: this.textures.barkBirch, roughness: 0.82 });
    const vineMat = new THREE.MeshStandardMaterial({ color: 0x15803d, roughness: 0.7 });
    const leafMat = new THREE.MeshStandardMaterial({ map: this.textures.leavesGreen, roughness: 0.6 });
    const flowerBudMat = new THREE.MeshStandardMaterial({ color: 0xf472b6, roughness: 0.4, emissive: 0xf472b6, emissiveIntensity: 0.25 });

    // 1. Natural Gnarled Trunk with Base Roots
    const trunkGeo = new THREE.CylinderGeometry(0.32, 0.58, 4.2, 8);
    ...
    // Root spurs
    for (let r = 0; r < 4; r++) {
      const rAng = (r / 4) * Math.PI * 2;
      const rootGeo = new THREE.CylinderGeometry(0.12, 0.28, 1.2, 6);
      rootGeo.translate(0, 0.6, 0);
      const root = new THREE.Mesh(rootGeo, barkMat);
      root.position.set(Math.cos(rAng) * 0.45, 0, Math.sin(rAng) * 0.45);
      root.rotation.x = Math.sin(rAng) * 0.5;
      root.rotation.z = -Math.cos(rAng) * 0.5;
      willow.add(root);
    }

    // 2. Spiral Ivy Vines Tightly Hugging the Main Trunk Surface
    const vineSteps = 42;
    for (let i = 0; i < vineSteps; i++) {
      const t = i / vineSteps;
      const tNext = (i + 1) / vineSteps;
      const angle = t * Math.PI * 7.0;
      const angleNext = tNext * Math.PI * 7.0;

      const r = 0.34 + (1 - t) * 0.24; // Matches trunk radius taper
      const rNext = 0.34 + (1 - tNext) * 0.24;

      const p1 = new THREE.Vector3(Math.cos(angle) * (r + 0.035), t * 4.0 + 0.1, Math.sin(angle) * (r + 0.035));
      const p2 = new THREE.Vector3(Math.cos(angleNext) * (rNext + 0.035), tNext * 4.0 + 0.1, Math.sin(angleNext) * (rNext + 0.035));

      const dir = new THREE.Vector3().subVectors(p2, p1);
      const segLen = dir.length();
      const vineSegGeo = new THREE.CylinderGeometry(0.045, 0.045, segLen, 5);
      vineSegGeo.translate(0, segLen / 2, 0);
      const vineSeg = new THREE.Mesh(vineSegGeo, vineMat);
      vineSeg.position.copy(p1);
      vineSeg.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir.clone().normalize());
      willow.add(vineSeg);

      // Clinging Ivy Leaves & Blossom Buds along the vine
      if (i % 2 === 0) {
        const leaf = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.03, 0.14), leafMat);
        leaf.position.copy(p1);
        leaf.rotation.set(Math.random() * 2, angle, Math.random() * 2);
        willow.add(leaf);
      }

      if (i % 7 === 0) {
        const bud = new THREE.Mesh(new THREE.SphereGeometry(0.09, 5, 5), flowerBudMat);
        bud.position.copy(p1);
        willow.add(bud);
      }
    }

    // 3. Spreading Continuous Branches with Solid Knuckle Joints
    const branchCount = 6;
    for (let b = 0; b < branchCount; b++) {
      const bAng = (b / branchCount) * Math.PI * 2 + (b % 2) * 0.25;
      const branchGroup = new THREE.Group();
      branchGroup.position.set(0, 4.0, 0);

      // Knuckle joint sphere at origin of branch
      const knuckle = new THREE.Mesh(new THREE.SphereGeometry(0.24, 7, 7), barkMat);
      branchGroup.add(knuckle);

      // Branch limb arching outward
      const limbLen = 2.6;
      const limbGeo = new THREE.CylinderGeometry(0.12, 0.22, limbLen, 7);
      limbGeo.translate(0, limbLen / 2, 0);
      const limbMesh = new THREE.Mesh(limbGeo, barkMat);
      limbMesh.castShadow = true;

      // Rotate branch outward and slightly downward
      branchGroup.rotation.y = bAng;
      branchGroup.rotation.z = 1.15; // Slants outward
      branchGroup.rotation.x = (Math.random() - 0.5) * 0.2;
      branchGroup.add(limbMesh);

      // 4. Vines Tightly Wrapped Along the Branch Limb
      const bVineSteps = 16;
      for (let bv = 0; bv < bVineSteps; bv++) {
        const bt = bv / bVineSteps;
        const vAngle = bt * Math.PI * 4.0;
        const brR = 0.22 - bt * 0.10;
        const vp1 = new THREE.Vector3(Math.cos(vAngle) * (brR + 0.025), bt * limbLen, Math.sin(vAngle) * (brR + 0.025));
        const vp2 = new THREE.Vector3(Math.cos(vAngle + 0.4) * (brR + 0.025), (bt + 1 / bVineSteps) * limbLen, Math.sin(vAngle + 0.4) * (brR + 0.025));

        const vDir = new THREE.Vector3().subVectors(vp2, vp1);
        const vLen = vDir.length();
        const bVineSegGeo = new THREE.CylinderGeometry(0.035, 0.035, vLen, 4);
        bVineSegGeo.translate(0, vLen / 2, 0);
        const bVineSeg = new THREE.Mesh(bVineSegGeo, vineMat);
        bVineSeg.position.copy(vp1);
        bVineSeg.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), vDir.clone().normalize());
        branchGroup.add(bVineSeg);

        if (bv % 2 === 0) {
          const bLeaf = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.025, 0.12), leafMat);
          bLeaf.position.copy(vp1);
          bLeaf.rotation.set(Math.random() * 2, vAngle, Math.random() * 2);
          branchGroup.add(bLeaf);
        }
      }

      // 5. Hanging Willow Foliage Tendrils (Anchored DIRECTLY to the wooden limb tip)
      const tipGroup = new THREE.Group();
      tipGroup.position.set(0, limbLen, 0);

      // Tendril knuckle cap
      const tipCap = new THREE.Mesh(new THREE.SphereGeometry(0.14, 6, 6), leafMat);
      tipGroup.add(tipCap);

      const subTendrilCount = 3;
      for (let st = 0; st < subTendrilCount; st++) {
        const tendril = new THREE.Group();
        const stAng = (st / subTendrilCount) * Math.PI * 2;
        tendril.position.set(Math.cos(stAng) * 0.15, 0, Math.sin(stAng) * 0.15);

        // Counteract branch tilt so tendril hangs vertically toward the ground
        tendril.rotation.z = -1.15;
        tendril.rotation.y = -bAng;

        const segs = 6 + (b % 3) * 2;
        for (let s = 0; s < segs; s++) {
          const segGeo = new THREE.CylinderGeometry(0.11 - s * 0.012, 0.14 - s * 0.012, 0.42, 5);
          segGeo.translate(0, -0.21, 0);
          const seg = new THREE.Mesh(segGeo, leafMat);
          seg.position.y = -s * 0.38;
          seg.rotation.z = (Math.random() - 0.5) * 0.08;
          tendril.add(seg);

          // Hanging blossom bud
          if (s === segs - 1 || s % 3 === 0) {
            const flower = new THREE.Mesh(new THREE.SphereGeometry(0.12, 5, 5), flowerBudMat);
            flower.position.set(0, -s * 0.38 - 0.22, 0);
            tendril.add(flower);
          }
        }
        tipGroup.add(tendril);
      }

      branchGroup.add(tipGroup);
      willow.add(branchGroup);
    }
  ...
  // ==========================================
  // 20 ANIMATED PATROLLING 3D NFT CHARACTERS (SOLID GROUND PATHS)
  // ==========================================
  buildCharacterPedestals() {
    // 20 Unique NFT Warriors deployed on distinct open, non-colliding paths (bridge lanes, plazas, shrine, ridge)
    const characterConfigs = [
      // 1-2: Arched Wooden Bridge (Left & Right separate lanes, strictly crossing bridge deck)
      { token: 1, color: 0xf59e0b, pA: [-5.0, 0, 0.75], pB: [5.0, 0, 0.75], speed: 0.38, name: 'Ronin Prime' },
      { token: 42, color: 0xa855f7, pA: [5.0, 0, -0.75], pB: [-5.0, 0, -0.75], speed: 0.36, name: 'Shadow Shinobi' },

      // 3-6: West Village Main Cobblestone Plaza (Open streets between x: -11 to -7, z: 2 to 18)
      { token: 12, color: 0xf59e0b, pA: [-8.5, 0, 3.5], pB: [-8.5, 0, 10.5], speed: 0.34, name: 'Sylvan Blade' },
      { token: 100, color: 0x0ea5e9, pA: [-10.5, 0, 1.5], pB: [-10.5, 0, 8.5], speed: 0.32, name: 'River Knight' },
      { token: 404, color: 0xf59e0b, pA: [-8.0, 0, 11.5], pB: [-8.0, 0, 17.5], speed: 0.35, name: 'Ronin Elder' },
      { token: 155, color: 0x10b981, pA: [-11.0, 0, 11.0], pB: [-11.0, 0, 17.0], speed: 0.31, name: 'Forest Scout' },

      // 7-10: East Village Garden Promenade (Open streets between x: 8 to 12, z: 2 to 18)
      { token: 69, color: 0x0ea5e9, pA: [8.5, 0, 2.5], pB: [8.5, 0, 9.5], speed: 0.35, name: 'River Vanguard' },
      { token: 200, color: 0x0ea5e9, pA: [10.5, 0, 1.5], pB: [10.5, 0, 7.5], speed: 0.33, name: 'Azure Tide' },
      { token: 666, color: 0xf59e0b, pA: [8.5, 0, 11.5], pB: [8.5, 0, 17.5], speed: 0.37, name: 'Solar Sentinel' },
      { token: 333, color: 0x10b981, pA: [11.0, 0, 11.0], pB: [11.0, 0, 17.0], speed: 0.33, name: 'Verdant Walker' },

      // 11-13: North-West Ancient Cobblestone Trail (x: -16 to -8, z: -16 to -2)
      { token: 500, color: 0xa855f7, pA: [-15.5, 0, -2.5], pB: [-9.5, 0, -2.5], speed: 0.34, name: 'Twilight Assassin' },
      { token: 777, color: 0x10b981, pA: [-7.8, 0, -3.5], pB: [-7.8, 0, -9.0], speed: 0.32, name: 'Canopy Warden' },
      { token: 1500, color: 0x10b981, pA: [-14.5, 0, -12.0], pB: [-14.5, 0, -6.5], speed: 0.30, name: 'Cliff Stalker' },

      // 14-16: North-East Orchard Pathway (x: 8 to 16, z: -16 to -2)
      { token: 1111, color: 0xa855f7, pA: [8.2, 0, -3.5], pB: [8.2, 0, -9.5], speed: 0.33, name: 'Void Watcher' },
      { token: 1888, color: 0x0ea5e9, pA: [10.5, 0, -10.5], pB: [16.5, 0, -10.5], speed: 0.35, name: 'Ridge Knight' },
      { token: 1999, color: 0xf59e0b, pA: [14.0, 0, -2.0], pB: [14.0, 0, -7.5], speed: 0.34, name: 'Genesis Champion' },

      // 17-19: Torii Shrine Sanctuary Courtyard (Elevated stone courtyard y = 1.4)
      { token: 888, color: 0xa855f7, pA: [-3.5, 0, -12.5], pB: [-3.5, 0, -16.5], speed: 0.36, name: 'Shrine Priest' },
      { token: 999, color: 0xf59e0b, pA: [3.5, 0, -12.5], pB: [3.5, 0, -16.5], speed: 0.36, name: 'Solar Guardian' },
      { token: 1234, color: 0x0ea5e9, pA: [-2.5, 0, -17.5], pB: [2.5, 0, -17.5], speed: 0.35, name: 'Crystal Keeper' },

      // 20: Mountain Windmill Terrace Overlook
      { token: 1777, color: 0xf59e0b, pA: [-21.5, 0, -2.0], pB: [-17.0, 0, -2.0], speed: 0.32, name: 'Mountain Ronin' }
    ];

    characterConfigs.forEach(cfg => {
      const group = new THREE.Group();
      const initialY = this.getGroundElevation(cfg.pA[0], cfg.pA[2]);
      group.position.set(cfg.pA[0], initialY, cfg.pA[2]);

      // Soft Natural Ground Contact Shadow (No bright artificial rings/wheels)
      ...
      this.drawCharacterOntoCanvas(cardCtx, cfg.token);
      ...
      const charPlane = new THREE.Mesh(new THREE.PlaneGeometry(2.1, 2.1), charMat);
      charPlane.position.y = 1.15;
      group.add(charPlane);

      // Soft Character Pointlight
      const charLight = new THREE.PointLight(cfg.color, 1.1, 4.5);
      charLight.position.set(0, 1.2, 0);
      group.add(charLight);
      ...
      group.userData = {
        ...
        token: cfg.token,
        pA: new THREE.Vector3(cfg.pA[0], 0, cfg.pA[2]),
        pB: new THREE.Vector3(cfg.pB[0], 0, cfg.pB[2]),
        speed: cfg.speed,
        baseY: 1.15,
        walkFreq: 5.5 + Math.random() * 2.0,
        phase: Math.random() * Math.PI * 2
      };

      this.characterPedestals.push(group);
      this.charactersGroup.add(group);
    });
  }

  drawCharacterOntoCanvas(ctx, tokenId) {
    if (typeof generateTraits !== 'undefined' && typeof NFTRenderer !== 'undefined') {
      try {
        ...
        const renderer = new NFTRenderer(tempCanvas);
    ...
    ctx.fillText('#' + tokenId, 256, 275);
  }

  // ==========================================
  // GRAND 3D PROJECT BILLBOARD ("BLOCKBIT INK")
  // ==========================================
  buildGrandProjectBillboard() {
    const billboardGroup = new THREE.Group();
    // Positioned prominently on the Northern Mountain Vista overlooking the village
    billboardGroup.position.set(0, 7.5, -21.5);

    const woodDarkMat = new THREE.MeshStandardMaterial({ map: this.textures.woodDark, roughness: 0.8 });
    const goldMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, metalness: 0.85, roughness: 0.15 });

    // 1. Two Sturdy Timber Support Pillars with Stone Footings
    const pillarL = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.48, 9.0, 8), woodDarkMat);
    pillarL.position.set(-6.5, 0, 0);
    pillarL.castShadow = true;
    billboardGroup.add(pillarL);

    const stoneFootL = new THREE.Mesh(new THREE.BoxGeometry(1.4, 1.2, 1.4), new THREE.MeshStandardMaterial({ map: this.textures.cobblestone, roughness: 0.85 }));
    stoneFootL.position.set(-6.5, -4.5, 0);
    billboardGroup.add(stoneFootL);

    const pillarR = pillarL.clone();
    pillarR.position.set(6.5, 0, 0);
    billboardGroup.add(pillarR);

    const stoneFootR = stoneFootL.clone();
    stoneFootR.position.set(6.5, -4.5, 0);
    billboardGroup.add(stoneFootR);

    // 2. Cross Beams & Frame
    const beamBottom = new THREE.Mesh(new THREE.BoxGeometry(15.2, 0.5, 0.5), woodDarkMat);
    beamBottom.position.set(0, -2.1, 0);
    billboardGroup.add(beamBottom);

    const beamTop = new THREE.Mesh(new THREE.BoxGeometry(16.0, 0.6, 0.6), woodDarkMat);
    beamTop.position.set(0, 2.6, 0);
    billboardGroup.add(beamTop);

    // 3. Ornate Japanese Curved Shingle Roof Eaves
    const roofShingleMat = new THREE.MeshStandardMaterial({ map: this.textures.roofPurple, roughness: 0.65 });
    const roofEave = new THREE.Mesh(new THREE.BoxGeometry(16.8, 0.4, 2.0), roofShingleMat);
    roofEave.position.set(0, 3.1, 0.25);
    roofEave.rotation.x = 0.18;
    roofEave.castShadow = true;
    billboardGroup.add(roofEave);

    const ridge = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 17.2, 8), goldMat);
    ridge.rotation.z = Math.PI / 2;
    ridge.position.set(0, 3.4, 0);
    billboardGroup.add(ridge);

    // 4. Two Hanging Glowing Lanterns
    const lanternL = this.createOrnateLantern(0xf59e0b);
    lanternL.position.set(-7.2, 1.8, 0.6);
    billboardGroup.add(lanternL);

    const lanternR = this.createOrnateLantern(0x38bdf8);
    lanternR.position.set(7.2, 1.8, 0.6);
    billboardGroup.add(lanternR);

    // 5. Billboard Display Canvas with 3D Styled Typography
    const bCanvas = document.createElement('canvas');
    bCanvas.width = 2048;
    bCanvas.height = 680;
    const ctx = bCanvas.getContext('2d');

    // Background Gradient (Deep Royal Midnight Twilight)
    const bgGrad = ctx.createLinearGradient(0, 0, 0, 680);
    bgGrad.addColorStop(0.0, '#0c0418');
    bgGrad.addColorStop(0.5, '#1e0c38');
    bgGrad.addColorStop(1.0, '#080210');
    ctx.fillStyle = bgGrad;
    ctx.fillRect(0, 0, 2048, 680);

    // Cyber / Fantasy Gold Frame Border
    ctx.strokeStyle = '#f59e0b';
    ctx.lineWidth = 14;
    ctx.strokeRect(16, 16, 2048 - 32, 680 - 32);

    ctx.strokeStyle = '#38bdf8';
    ctx.lineWidth = 4;
    ctx.strokeRect(28, 28, 2048 - 56, 680 - 56);

    // Subtle Tech Grid / Runes
    ctx.strokeStyle = 'rgba(168, 85, 247, 0.18)';
    ctx.lineWidth = 2;
    for (let gx = 60; gx < 2048; gx += 80) {
      ctx.beginPath();
      ctx.moveTo(gx, 40);
      ctx.lineTo(gx, 640);
      ctx.stroke();
    ...
    // Top Sub-Badge: [ INK SUPERCHAIN L2 • POWERED BY KRAKEN ]
    ctx.fillStyle = '#fbbf24';
    ctx.font = 'bold 36px "Outfit", "Space Grotesk", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('⚡ INK SUPERCHAIN L2 • POWERED BY KRAKEN ⚡', 1024, 110);

    // 3D Styled Main Title: BLOCKBIT (White with Cyan/Blue 3D bevel) + INK (Purple/Cyan glow)
    const titleY = 380;
    ctx.font = '900 165px "Outfit", "Cinzel", sans-serif';
    ctx.textAlign = 'left';

    const blockbitMetrics = ctx.measureText('BLOCKBIT ');
    const inkMetrics = ctx.measureText('INK');
    const totalTitleWidth = blockbitMetrics.width + inkMetrics.width;
    const startX = (2048 - totalTitleWidth) / 2;

    // Draw 3D Extrusion Shadow Layers for BLOCKBIT (Cyan / Royal Blue Bevel)
    for (let s = 18; s >= 1; s--) {
      ctx.fillStyle = s > 10 ? '#0369a1' : '#0284c7';
      ctx.fillText('BLOCKBIT', startX + s * 0.8, titleY + s * 1.2);
    }
    // Main Crisp Pure White Face
    ctx.fillStyle = '#ffffff';
    ctx.shadowColor = '#38bdf8';
    ctx.shadowBlur = 24;
    ctx.fillText('BLOCKBIT', startX, titleY);
    ctx.shadowBlur = 0; // reset

    // Draw 3D Extrusion Shadow Layers for INK (Deep Velvet Violet / Royal Indigo)
    const inkX = startX + blockbitMetrics.width;
    for (let s = 18; s >= 1; s--) {
      ctx.fillStyle = s > 10 ? '#3b0764' : '#6b21a8';
      ctx.fillText('INK', inkX + s * 0.8, titleY + s * 1.2);
    }
    // Main Glowing Purple Face with Cyan Rim
    ctx.fillStyle = '#c084fc';
    ctx.shadowColor = '#a855f7';
    ctx.shadowBlur = 35;
    ctx.fillText('INK', inkX, titleY);
    ctx.shadowBlur = 0;

    // Bottom Ribbon Subtitle: 1,999 GENERATIVE ANIME WARRIORS • MINT & WHITELIST
    ctx.fillStyle = '#e2e8f0';
    ctx.font = '600 40px "Plus Jakarta Sans", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('1,999 UNIQUE ANIME WARRIORS  •  SACRED ENCHANTED REALM', 1024, 530);

    ctx.fillStyle = '#f59e0b';
    ctx.font = 'bold 32px "Space Grotesk", monospace';
    ctx.fillText('[ WHITELIST RAFFLE LIVE NOW ]', 1024, 600);

    const bTexture = new THREE.CanvasTexture(bCanvas);
    bTexture.minFilter = THREE.LinearFilter;
    bTexture.magFilter = THREE.LinearFilter;
    ...
    const boardMesh = new THREE.Mesh(new THREE.BoxGeometry(14.2, 4.6, 0.25), boardMat);
    boardMesh.position.set(0, 0.25, 0.15);
    boardMesh.castShadow = true;
    billboardGroup.add(boardMesh);

    // Warm Golden-Cyan Spotlight Illumination on Billboard
    const bLight1 = new THREE.PointLight(0x38bdf8, 2.8, 16);
    bLight1.position.set(-4.5, 1.8, 3.5);
    billboardGroup.add(bLight1);

    const bLight2 = new THREE.PointLight(0xf59e0b, 2.8, 16);
    bLight2.position.set(4.5, 1.8, 3.5);
    billboardGroup.add(bLight2);

    this.buildingsGroup.add(billboardGroup);
  }
